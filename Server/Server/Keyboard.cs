...
namespace Server
{
    class Keyboard
    {
        private static int KEY_DOWN = 0x01;
        private static int KEY_UP = 0x02;
        ...
        internal static void keyboardControl(byte[] command)
        {
            int action= BinaryPrimitives.ReadInt32BigEndian(new Span<byte>(command, 1, 4));
            if (action == KEY_DOWN) {
                int val = BinaryPrimitives.ReadInt32BigEndian(new Span<byte>(command, 5, 4)); 
                SendKeyDown(val);
            }
        ...
        /// <summary>
        /// Determines if the <see cref="VirtualKeyShort"/> is an ExtendedKey
        /// </summary>
        ...
        private static void SendKeyDown(int code)
        {
            ...
            Input.Data.Keyboard = new KEYBDINPUT()
            {
                ...
                //wScan = (ScanCodeShort)code,
                wVk = (VirtualKeyShort)code,
                //wVk=VirtualKeyShort.OEM_1,
            };
            Inputs[0] = Input;
            SendInput(1, Inputs, Marshal.SizeOf(typeof(INPUT)));
        }
        private static void SendKeyUp(int code)
        {
            var Inputs = new INPUT[1];
            var Input = new INPUT();
            Input.Type = 1;
